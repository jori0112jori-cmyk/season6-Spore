/* --- Static Data --- */
const DEFAULT_DATA = {
    COSTS: [0, 700, 11200, 22400, 44800, 89600, 125440, 163073, 195686, 234824, 281788, 338146, 405776, 486931, 584317, 701180, 771299, 848428, 933270, 1026598, 1129258, 1242183, 1366401, 1503041, 1653346, 1818680, 919100, 928300, 937600, 947000, 956500, 966000, 975700, 985500, 995300, 1005300, 1206300, 1326900, 1333600, 1340200, 1346900, 1353700, 1360400, 1367300, 1374100, 1381000, 1387900, 1394800, 1401800, 1408800, 1415800, 1422900, 1430000, 1436700, 1444400, 1451600, 1458800, 1466100, 1473500, 1480800, 1488200],
...
const app = (() => {
    ...
    const validateStock = () => {
        ...
        if(!el) return;
        // k/m以外の数字・小数点以外は入力させない、かつk/mは大文字に統一
        el.value = el.value.replace(/[^0-9.kKmM]/g, '').toUpperCase();
    };
